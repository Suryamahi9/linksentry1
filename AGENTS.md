...
LinkSentry scans URLs for phishing/malware/spam and returns an explainable 0-100 risk score. Two independent packages, each with its own `package.json` + lockfile. No root test suite, no CI, no git repo, no API-side linter.
...
- Next.js rewrites same-origin `/api/*` → `API_UPSTREAM` via `next.config.js` — but **only when an upstream is set** (`NODE_ENV !== "production"` falls back to `http://localhost:3001` for dev). On Vercel the Fastify function is served same-origin at `/api/*`, so `API_UPSTREAM` is left unset and no rewrite/CORS is involved. Browser code calls `/api/v1/scan` (`src/lib/api.ts`); never call the API cross-origin from the browser.
- API CORS allow-list is `CORS_ORIGIN` (comma-separated, defaults `localhost:3000,localhost:8000`). Set `API_UPSTREAM` at build-time on Vercel, and add the Vercel domain (e.g. `app.vercel.app`) to `CORS_ORIGIN` on the API — the deployed site's browser requests will be blocked otherwise.
