...
- Prisma schema lives only at `api/prisma/schema.prisma` (Postgres). After edits, run `npx prisma generate` (and `npx prisma db push`/migrate) inside `api/`. The web app's `src/lib/prisma.ts` deliberately uses a runtime `require()` so the frontend typechecks without a generated client — don't hard-import `@prisma/client` in the web app. The API's `api/src/lib/prisma.ts` is also graceful: it guards `new PrismaClient()` in try/catch and returns `null` when `POSTGRES_URL` is unset or the client isn't generated, so the API boots DB-less — do NOT re-add a hard constructor call.
